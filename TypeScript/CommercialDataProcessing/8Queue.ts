/*********************************************************************************************************
 * @purpose	:maintain dateTime of the transaction in a queue implemented using linked list to indicate when the transactions were done
 * 
...
class OperationStack{
    ...
    /**read transaction json file and store data in transactionArray */
    ...
    buy(symbol: string, noOfShares: number, personId: number): any {
        try {
            ...
            if (tc1 == 'success' && tc2 == 'success') {
                ...
                let newnode=new OperationNode(personId,symbol,noOfShares,"BUY",dt);
                 /**call insert at end method of linked list */
                this.linkedlistObj.insertAtEnd(newnode);
                return 'success';
            }
        } catch (e) {
            console.log(e);
        ...
        try {
            ...
            if (isFound == true) {
                ...
                if (tc1 == 'success' && tc2 == 'success') {
                    ...
                    let newnode=new OperationNode(personId,symbol,noOfShares,"SELL",dt);
                    /**call insert at end method of linked list */
                    this.linkedlistObj.insertAtEnd(newnode);
                    return 'success';
                }
    ...
    mainMethod(){
        try{
            ...
            this.readPersonJsonArray();
            this.readTrasactionJsonArray();
            /**create queue of transaction array */
            for(let i=0;i<this.transactionArray.length;i++)
            {
               let personId=this.transactionArray[i]['personId'];
               let symbol= this.transactionArray[i]['symbol'];
               let noOfShares=this.transactionArray[i]['noOfShares'];
               let operation=this.transactionArray[i]['operation'];
               let dateTime=this.transactionArray[i]['dateTime'];
               /**create new node */
               let newnode=new OperationNode(personId,symbol,noOfShares,operation,dateTime);
                ...
                switch(choice)
                {
                    case 1:{
                        /**get information from console from console */
                            ...
                            let tc1 = this.buy(symbol, noofshares, pid);//call buy()
                    ...
                    case 2:{
                        /**get information for sell opeartion from console */
                            ...
                            let noofshares = utility.getNumber();
                            let tc1 = this.sell(symbol, noofshares, pid);//call sell() method
